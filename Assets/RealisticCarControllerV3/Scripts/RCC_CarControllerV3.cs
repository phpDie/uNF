...
public class RCC_CarControllerV3 : MonoBehaviour {
	...
	public float currentSteerAngle = 40f;		// Maximum Steer Angle Of Your Vehicle.
    ...
    public float antiTestRollFrontHorizontal = 77700f;		// Anti Roll Horizontal Force For Preventing Flip Overs And Stability.
	...
	[HideInInspector]public GameObject chassis;      // Script Will Simulate Chassis Movement Based On Vehicle Rigidbody Velocity.
        ...
        void Start()
    {
       ...
       // print("RCC Start");
		...
		orgSteerAngle = currentSteerAngle;
...
//			AIController = true;
        ...
        if (!chassis)
        {
            chassis = gameObject.transform.Find("Chassis").gameObject;
        }
        ...
        if (gameObject.GetComponent<EngineConverter>())if(gameObject.GetComponent<EngineConverter>().enabled)
        {
             //  print("[EngineConverter] Забираю данные");
            EngineBridgeStruct Brige = gameObject.GetComponent<EngineConverter>().GenerateBrige();
                
                highspeedsteerAngle = Brige.highspeedsteerAngle;
                highspeedsteerAngleAtspeed = Brige.highspeedsteerAngleAtspeed;
                engineTorque = Brige.engineTorque;
                maxspeed = Brige.maxspeed;

                antiRollFrontHorizontal = Brige.antiRollFrontHorizontal;
                antiRollRearHorizontal = Brige.antiRollFrontHorizontal;   //не ошибка!

                rigid.maxAngularVelocity = Brige.maxAngularVelocity;
                rigid.mass = Mathf.CeilToInt(Brige.weight);

            }
            else
            {
                print("[EngineConverter] Не найден конвертер!!!");
                if (gameObject.GetComponent<EngineConverter>())
                {
                    print("Но по факту EngineConverter есть ");
                }
            }
        ...
        foreach (RCC_WheelCollider item  in allWheelColliders)
        {
            item.Init();
        }
	...
	public IEnumerator StartEngineDelayed (){
		...
		engineRunning = false;
		engineStartSound = RCC_CreateAudioSource.NewAudioSource(gameObject, "Engine Start AudioSource", 5, 10, 1, engineStartClip, false, false, true);
...
//				rigid.AddRelativeTorque (Vector3.up * (((steerInput * _gasInput) * 10000f)), ForceMode.Force); 
	...
	void Engine (){
		...
		//Steer Limit.
		currentSteerAngle = Mathf.Lerp(orgSteerAngle, highspeedsteerAngle, (speed / highspeedsteerAngleAtspeed));
