...
public class RCC_Chassis : MonoBehaviour {
	...
	void ChassisJoint(){
        ...
        if (!GetComponentInParent<RCC_CarControllerV3>().enabled) return;
        if (!GetComponentInParent<RCC_CarControllerV3>().chassis) return;
		...
		foreach(Transform t in childTransforms){
			...
			if(t.gameObject.activeSelf && t.GetComponent<Collider>()){
                if(t.transform.parent.name== "VISUAL_TUNE")
                {
                    continue;
                }
				if (t.childCount >= 1) {
