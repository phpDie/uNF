   ...
   public void Init()
    {
        ...
        bool USE_ENGINE_BRIGE = false;
        if (carController.GetComponent<EngineConverter>()) if (carController.GetComponent<EngineConverter>().enabled)
            {
                USE_ENGINE_BRIGE = true;
            }
       // USE_ENGINE_BRIGE = false;
        ...
        { 
                ...
                //print("[RCC_WHEEL] Забираю данные");
                WheelBridgeStruct Brige = carController.GetComponent<EngineConverter>().GenerateWheelBrige();

                float _DRIFT= (Brige.driftval + 1f) / 2f;
            
                  forwardFrictionCurve = SetFrictionCurves(forwardFrictionCurve, 
                     Mathf.Lerp(0.18f,0.3f, _DRIFT),
                      Mathf.Lerp(1.4f, 0.86f, _DRIFT), 
                     .8f,
                      Mathf.Lerp(1.15f, .56f, _DRIFT));
               

            sidewaysFrictionCurve = SetFrictionCurves(sidewaysFrictionCurve,
                    Mathf.Lerp(.25f, 3.7f, _DRIFT), 1f, .5f, .75f);
	...
	public void ApplySteering(){
		...
		if(carController.applyCounterSteering && carController.currentGear != 0)
			wheelCollider.steerAngle = Mathf.Clamp((carController.currentSteerAngle * (carController._steerInput + carController.driftAngle)), -carController.currentSteerAngle, carController.currentSteerAngle);
		else
			wheelCollider.steerAngle = Mathf.Clamp((carController.currentSteerAngle * carController._steerInput), -carController.currentSteerAngle, carController.currentSteerAngle);
