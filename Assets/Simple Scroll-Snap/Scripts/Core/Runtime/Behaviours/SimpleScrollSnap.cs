...
namespace DanielLochner.Assets.SimpleScrollSnap
{
    ...
    public class SimpleScrollSnap : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        private void Setup()
        {
            ...
            Vector2 sizeReult = size;
            // Panels
            if (sizeControl == SizeControl.Manual)
            {
                sizeReult = size;
            }
            if (sizeControl == SizeControl.Fit)
            {
                sizeReult = Viewport.rect.size; 
            }
            if (sizeControl == SizeControl.Scaler)
            {
                sizeReult = Viewport.rect.size;

             //   print("SCALER MODE orig");
               // print(sizeReult);
                sizeReult = Viewport.rect.size * size;
               // print("SCALER MODE post:");
              //  print(sizeReult); 
            }
            ...
            for (int i = 0; i < NumberOfPanels; i++)
            {
                ...
                if (movementType == MovementType.Fixed && useAutomaticLayout)
                {
                    ...
                    Vector2 marginOffset = new Vector2(x / sizeReult.x, y / sizeReult.y);
                    Panels[i].pivot = new Vector2(0.5f, 0.5f) + marginOffset;
                    Panels[i].sizeDelta = sizeReult - new Vector2(automaticLayoutMargins.Left + automaticLayoutMargins.Right, automaticLayoutMargins.Top + automaticLayoutMargins.Bottom);

                    float panelPosX = (movementAxis == MovementAxis.Horizontal) ? i * (automaticLayoutSpacing + 1f) * sizeReult.x + (sizeReult.x / 2f) : 0f;
                    float panelPosY = (movementAxis == MovementAxis.Vertical)   ? i * (automaticLayoutSpacing + 1f) * sizeReult.y + (sizeReult.y / 2f) : 0f;
            ...
            if (movementType == MovementType.Fixed)
            {
                ...
                if (useAutomaticLayout)
                {
                    ...
                    float contentWidth  = (movementAxis == MovementAxis.Horizontal) ? (NumberOfPanels * (automaticLayoutSpacing + 1f) * sizeReult.x) - (sizeReult.x * automaticLayoutSpacing) : sizeReult.x;
                    float contentHeight = (movementAxis == MovementAxis.Vertical)   ? (NumberOfPanels * (automaticLayoutSpacing + 1f) * sizeReult.y) - (sizeReult.y * automaticLayoutSpacing) : sizeReult.y;
                ...
                if (useInfiniteScrolling)
                {
                    ...
                    contentSize = Content.rect.size + (sizeReult * infiniteScrollingSpacing);
